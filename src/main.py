...
# Public identity is intentionally not configurable so stale deployment
# variables cannot generate links to a retired bot account.
...
app = FastAPI(title="ChessMeet", version="1.5.1", lifespan=lifespan)
    ...
    if content is None:
        def fetch_tile() -> bytes:
            request = Request(
                ...
                headers={"User-Agent": "ChessMeet/1.5.1 (Telegram Mini App; contact: @chessmeetbot)"},
            )
