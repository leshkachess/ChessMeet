# ChessMeet v1.0.0 — security and stability audit

## Новый пользовательский путь v1.0

- Поиск фильтруется по формату, уровню, дате и наличию доски.
- Отклик отправляется через отдельную форму без системных `prompt()`.
- Пустой город получает сценарий первой заявки и подписки.
- Участники подтверждённой встречи могут отметить «Я на месте».
- Telegram Back Button и Haptic Feedback интегрированы в основные действия.
- Ключевые действия записываются в локальную таблицу продуктовой аналитики.
